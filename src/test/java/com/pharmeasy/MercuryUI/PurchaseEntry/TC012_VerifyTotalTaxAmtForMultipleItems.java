...
public class TC012_VerifyTotalTaxAmtForMultipleItems extends TestBase{

	public static final Logger log = Logger.getLogger(TC012_VerifyTotalTaxAmtForMultipleItems.class.getSimpleName());
	
	@Test
	public void verifyTotalTaxAmt() throws InterruptedException {
		...
		double calculatedTotalTaxAmt = new BigDecimal(purchaseEntry.getCalculatedFooterDetailsFromItemTable().get("Total Tax")).setScale(2, RoundingMode.HALF_UP).doubleValue();
		
		Assert.assertEquals(purchaseEntry.getFooterDetails().get("Total Tax"), calculatedTotalTaxAmt);
